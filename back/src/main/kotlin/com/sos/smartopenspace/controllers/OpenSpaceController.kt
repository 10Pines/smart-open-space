...
class OpenSpaceController(
  ...
  fun findAllByUser(@PathVariable userID: Long) =
    openSpaceService.findAllByUser(userID)
      .map { OpenSpaceTranslator.translateFrom(it) }
  ...
  @LoggingInputExecution
  fun findAssignedSlotsById(@PathVariable id: Long) =
    openSpaceService.findAssignedSlotsById(id)

  @PutMapping("/activateQueue/{userID}/{osID}")
  @LoggingInputExecution
  fun activateQueue(@PathVariable userID: Long, @PathVariable osID: Long) =
    OpenSpaceTranslator.translateFrom(
      openSpaceService.activateQueue(
        userID,
        osID
      )
    )
  ...
  fun callForPapers(
    ...
    OpenSpaceTranslator.translateFrom(
      openSpaceService.toggleCallForPapers(
        openSpaceId,
        userID
      )
    )

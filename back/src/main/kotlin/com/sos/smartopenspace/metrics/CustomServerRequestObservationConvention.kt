    ...
    when (val apiError: Any? = context.get(API_ERROR_CONTEXT_FIELD)) {
      is DefaultErrorDto -> {
        keyValues = keyValues.and(
          TAG_IS_FALLBACK_ERROR,
          apiError.isFallbackError.toString(),
          TAG_ERROR_CODE,
          apiError.statusCode.toString(),
          TAG_ERROR_NAME,
          getAtMaxWidthOrEmptyValueIfBlank(apiError.status),
          TAG_ERROR_MESSAGE,
          getAtMaxWidthOrEmptyValueIfBlankOrNull(apiError.message),
        )
      }

...
@Transactional
class OpenSpaceControllerTest : BaseIntegrationTest() {
  ...
  fun `creating a valid OpenSpace returns an ok status response`() {
    ...
    mockMvc.perform(
      ...
      .andExpect(
        MockMvcResultMatchers.jsonPath("$.isActiveCallForPapers").value(false)
      )
      .andExpect(
        MockMvcResultMatchers.jsonPath("$.tracks[0].color").value(track_color)
      )
      .andExpect(
        MockMvcResultMatchers.jsonPath("$.tracks[0].name").value(track_name)
      )
      .andExpect(
        MockMvcResultMatchers.jsonPath("$.tracks[0].description")
      ...
      .andExpect(
        MockMvcResultMatchers.jsonPath("$.showSpeakerName").value(true)
      )
      .andExpect(
        MockMvcResultMatchers.jsonPath("$.isActiveVoting").value(false)
      )
  }
  ...
  fun `deleting an OpenSpace with an invalid user returns a forbidden response`() {
    ...
    val (otherUser, bearerTokenOtherUser) = registerAndGenerateAuthToken(
      aUser(
  ...
  fun `can create a valid talk and get it correctly`() {
    ...
    val anOpenSpace =
      openSpaceRepository.save(anyOpenSpaceWith(user, setOf(track)))
    anOpenSpace.toggleCallForPapers(user)
    val aMeetingLink = "https://aLink"
    val aDocument =
      Document("a document", URI("https://www.lipsum.com/").toURL())

    mockMvc.perform(
      MockMvcRequestBuilders.post("/openSpace/talk/${user.id}/${anOpenSpace.id}")
        .contentType(MediaType.APPLICATION_JSON)
        .content(
          generateTalkWithTrackBody(
            aMeetingLink,
            track,
            document = aDocument
          )
    ...
    mockMvc.perform(
      ...
      .andExpect(
        MockMvcResultMatchers.jsonPath("$[0].meetingLink").value(aMeetingLink)
      )
      .andExpect(
        MockMvcResultMatchers.jsonPath("$[0].track.name").value(track.name)
      )
      .andExpect(
        MockMvcResultMatchers.jsonPath("$[0].documents[0].name")
          .value(aDocument.name)
      )
  }
  ...
  fun `can create a valid marketplace talk and get it correctly`() {
    val user = userRepo.save(aUser())
    val anOpenSpace = openSpaceRepository.save(anyOpenSpaceWith(user))
    anOpenSpace.toggleCallForPapers(user)
    ...
    mockMvc.perform(
      ...
      .andExpect(MockMvcResultMatchers.jsonPath("$[0].id").exists())
      .andExpect(
        MockMvcResultMatchers.jsonPath("$[0].isMarketplaceTalk").value(true)
      )
      .andExpect(
        MockMvcResultMatchers.jsonPath("$[0].speakerName").value("Alan Key")
      )
  }
  ...
  fun `creating an invalid talk return an bad request status`() {
    val user = userRepo.save(aUser())
    val anOpenSpace = openSpaceRepository.save(anyOpenSpace())
  ...
  fun `deleting a valid talk with organizer openspace return an ok status response`() {
    val (user, _) = registerAndGenerateAuthToken(aUser(userEmail = "user@gmail.com"))
    val (organizer, organizerBearerToken) = registerAndGenerateAuthToken(
      aUser(
  ...
  fun `deleting a valid talk with an invalid user return a forbidden response`() {
    val (user, _) = registerAndGenerateAuthToken(aUser(userEmail = "user@gmail.com"))
    val (otherUser, otherUserBearerToken) = registerAndGenerateAuthToken(
      aUser(
  ...
  fun `start a call for papers returns an ok status response and the modified Open Space`() {
    ...
    mockMvc.perform(
      ...
      .andExpect(
        MockMvcResultMatchers.jsonPath("$.isActiveCallForPapers").value(true)
      )
  }
  ...
  fun `starting a call for papers with a non organizer user return a forbidden status`() {
    ...
    val anOpenSpace = openSpaceRepository.save(anyOpenSpaceWith(organizer))
  ...
  fun `can update an open space name`() {
    ...
    val changedOpenSpace = openSpaceRepository.findById(anOpenSpace.id).get()
  ...
  fun `updating an invalid open space throws a not found response`() {
    val (someUser, someUserBearerToken) = registerAndGenerateAuthToken(
      aUser(
  ...
  fun `updating with not exist user and their token throws a unauthorized response`() {
    val (someUser, someUserBearerToken, someUserAuthSession) = registerAndGetAuthSession(
      aUser(userEmail = "someUser@gmail.com")
    )
  ...
  fun `toggle show speaker name returns an ok status response and the modified Open Space`() {
    ...
    mockMvc.perform(
      ...
      .andExpect(MockMvcResultMatchers.jsonPath("$.id").value(anOpenSpace.id))
      .andExpect(
        MockMvcResultMatchers.jsonPath("$.showSpeakerName").value(false)
      )
  }

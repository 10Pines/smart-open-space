...
class AuthControllerTest : BaseIntegrationTest() {
  ...
  fun `register with register request dto should be http ok and return AuthResponseDto`() {
    ...
    val userEmail =
      extractJsonPathValue<String>(registerReqDtoMockStr, "$.email")
        ?: fail("not email provided")
    val userPassword =
      extractJsonPathValue<String>(registerReqDtoMockStr, "$.password")
    ...
    val savedUser =
      userService.findUserAndMatchPassword(userEmail, userPassword)
  ...
  fun `register with a valid register request dto but email already exist should be http bad request`() {
    ...
    assertEquals(
      expectedErrMsg,
      extractJsonPathValue<String>(responseBodyStr, "$.message")
    )
    assertEquals(
      expectedErrStatusCode,
      extractJsonPathValue<Int>(responseBodyStr, "$.statusCode")
    )
    assertEquals(
      "bad_request",
      extractJsonPathValue<String>(responseBodyStr, "$.status")
    )
    assertEquals(
      false,
      extractJsonPathValue<Boolean>(responseBodyStr, "$.isFallbackError")
    )
  ...
  fun `register with register request dto should be http bad request`(
    ...
    val registerReqDtoMockStr =
      ReadMocksHelper.readAuthMocksFile(REGISTER_MOCKS_DIR + filename)
    // WHEN
    val httpResponse = mockMvc.perform(
      MockMvcRequestBuilders.post(REGISTER_ENDPOINT)
        .contentType(MediaType.APPLICATION_JSON)
  ...
  fun `test login with login request dto should be http ok and return AuthResponseDto`() {
    ...
    val emailRequest =
      extractJsonPathValue<String>(loginReqDtoMockStr, "$.email")
        ?: fail("not email provided")
    val passwordRequest =
      extractJsonPathValue<String>(loginReqDtoMockStr, "$.password")
        ?: fail("not email provided")
    createUserWithEmailAndPassword(emailRequest, passwordRequest)
    val savedUser =
      userService.findUserAndMatchPassword(emailRequest, passwordRequest)
  ...
  fun `test logout with valid token successful should return http ok and revoke current token`() {
    ...
    val bodyRes = httpResponse.andExpect(status().is2xxSuccessful)
      .andReturn().response.getContentAsString(StandardCharsets.UTF_8)
    val logoutRes =
      objectMapper.readValue(bodyRes, LogoutResponseDTO::class.java)
    assertEquals(SUCCESS_LOGOUT_MESSAGE, logoutRes.message)

    val allUserAuthSessions =
      authSessionRepo.findAllByUserId(authSessionSaved1.user.id)
    val postAuthSession1 =
      allUserAuthSessions.first { it.id == authSessionSaved1.id }
  ...
  fun `test purge invalid sessions should be return ok and count all deleted sessions`(
    ...
    val deletedSessionsResponseDTO =
      objectMapper.readValue(
        responseBodyStr,
        DeletedSessionsResponseDTO::class.java
      )
  ...
  fun `test purge invalid sessions should be return ok and not match any session`() {
    ...
    assertEquals(
      Instant.parse(createdOnFrom),
      deletedSessionsResponseDTO.creationDateFrom
    )
    assertEquals(
      Instant.parse(createdOnTo),
      deletedSessionsResponseDTO.creationDateTo
    )
  ...
  private fun createUserWithEmailAndPassword(
    ...
    val newUser =
      UserSampler.getWith(id = 0, email = email, password = password)
  ...
  companion object {
    ...
    private const val PURGE_INVALID_SESSIONS_ENDPOINT =
      "$AUTH_ENDPOINT/purge/invalid-sessions"
  }
}
